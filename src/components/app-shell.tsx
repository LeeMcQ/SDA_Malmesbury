...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative min-h-dvh bg-paper text-ink">
      ...
      <main className="relative mx-auto w-full min-w-0 max-w-5xl overflow-x-hidden pb-28">{children}</main>

      <nav
        className="fixed inset-x-0 bottom-0 z-30 border-t border-line/80 bg-paper/95 backdrop-blur-md"
        style={{ paddingBottom: "env(safe-area-inset-bottom)" }}
      >
        <ul className="mx-auto grid max-w-5xl grid-cols-4 px-2">
          {NAV.map((item) => {
            const active =
              item.to === "/"
